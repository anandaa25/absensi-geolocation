    ...
    <div class="row justify-content-center my-5">
        <div class="col-md-6 ">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="<?= base_url('cekPembatalan') ?>" method="POST">
                        <div class="form-group">
                            <label>Kode Pembayaran</label>
                            <input type="text" name='kode' class="form-control" placeholder="Kode Pembayaran">
                        </div>

                        <button class="btn font-weight-bold float-right" style="background-color: #8FBC8F">Cek Kode Pembayaran</button>
                    </form>
                </div>
            </div>
            <hr>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            ...
                            <?php if ($stat_pem['status'] === '3') : ?>
							<span style='color:red'><center><b>TIKET SUDAH DIBATALKAN</b></center></span>
                            <span style='color:red'><center><b>HARAP KONFIRMASI KEPADA ADMIN MELALUI TELFON</b></center></span>
                            <span style='color:red'><center><b>UNTUK PENGEMBALIAN DANA</b></center></span>
							<form onclick="return confirm('Yakin Batalkan Pembatalan Tiket?')" action="<?= base_url('batalkanTiket') ?>" method="post">
                                    <input type="hidden" name="kode_booking" value="<?= $_GET['kode'] ?>">
                                    <input type="hidden" name="batal_tiket" value="2">
                                    <input type="hidden" name="kursi_dipilih" value="<?= implode(',',$kursi_dipilih) ?>">
                                    <button type="submit" class="btn btn-block text-black" style="background-color: red">BATALKAN PEMBATALAN?</button>
                                </form>
							<?php endif; ?>
                            <?php if ($stat_pem['status'] === '2') : ?>
                                <form action="<?= base_url('batalkanTiket') ?>" onclick="return confirm('Yakin Batalkan Tiket?')" method="post">
                                    <input type="hidden" name="kode_booking" value="<?= $_GET['kode'] ?>">
                                    <input type="hidden" name="batal_tiket" value="3">
									<input type="hidden" name="kursi_dipilih" value="<?= implode(',',$kursi_dipilih) ?>">
                                    <button type="submit" class="btn btn-block text-black" style="background-color: yellow">BATALKAN TIKET?</button>
                                </form>
